...
static void tests() {
    ...
    std::vector<int64_t> arr_sorted2 = sorting::quick_sort::quick_sort(
        arr2, 0, int(std::end(arr2) - std::begin(arr2)) - 1);
    ...
    size_t size = std::rand() % 100 + 10; //std::rand() % 750 + 100;
...
void MaxandSecond(int a[], int left, int right, int& max, int& second)
{
    if (left == right)
    {
        max = a[left];
    ...
    else
    {
        ...
        int leftmax;
        int leftsecond;
        MaxandSecond(a, left, mid, leftmax, leftsecond);

        int rightmax;
        int rightsecond;
        MaxandSecond(a, mid + 1, right, rightmax, rightsecond);
...
static void testMaxandSecond() {
    int N = 10;
    int arr[] = { 5, 3, 8, 12, 14, 16, 28, 96, 2, 96};
    ...
    MaxandSecond(arr, 0, N - 1, max, second);

    std::cout << "\n Max is " << max << ", and second is " << second << "\n";

    int arr2[] = { 5, 3, 8, 12, 14, 16, 28, 96, 2, 9 };

    std::cout << "\n The array 2 is : ";
    for (int i = 0; i < N; i++) std::cout << arr2[i] << " ";

    MaxandSecond(arr2, 0, N - 1, max, second);

    std::cout << "\n Max is " << max << ", and second is " << second << "\n";

    int arr3[] = { 55, 3, 8, 12, 14, 16, 28, 96, 2};

    std::cout << "\n The array 3 is : ";
    for (int i = 0; i < N-1; i++) std::cout << arr3[i] << " ";

    MaxandSecond(arr3, 0, N - 2, max, second);
